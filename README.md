...
- Mac 清理接收文件时，会移除能确认归属且已为空的自动外层目录；非空目录和用户自建目录保持不动。
- 补齐 Mac 前台其他未读会话的通知显示，保留后台投递、静音和“正在阅读时不打扰”；共享屏幕期间是否显示由 macOS 全局通知设置决定。
- 普通聊天和临时会话的图片支持缩放、拖动和双指手势，并提供 100% 与适应窗口操作。
- 改善 Mac 最小化后的局域网响应，容忍一次短暂心跳超时，同时继续允许系统正常睡眠。
- Windows 保持 v0.10.1；图片缩放基于当前预览分辨率，不能恢复预览生成时已缩小的原始细节。
...
- Android APK 使用固定发布证书。后续覆盖升级由系统核对签名；[公开证书指纹](https://github.com/qe1233/LinkNear-Releases/releases/download/v0.10.14/android-signing-certificate.sha256)随安装包发布。
